...
```bash
# the first time only create data folder
mdkir __binds/postgresql/data
# start dependencies
make docker-dev
# start server
make server
...
- `startTimestamp` required, must be unix timestamps in seconds.
...
- `from` must be unix timestamps in seconds.
- `to` must be unix timestamps in seconds.
